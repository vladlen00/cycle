...
**Обновлён:** 9 мая 2026 (поздний вечер)
...
- **Легенда фаз календаря не была видна при открытии.** Календарь скролит к текущему месяцу (последний из трёх), легенда выше viewport. **Фикс:** `position: sticky` + `top: 0` + `z-index: 5` на `.calendar-legend`, `background: #ffffff` чтобы не просвечивало скроллящееся содержимое.

- **Календарь перешёл на новый визуальный язык.** Менструация - сплошной розовый круг 32x32 с белым числом (факт) или пунктирный круг с цветным числом (прогноз). Овуляция - контурное сердце SVG (stroke #b8932a) поверх числа, сплошной для факта, stroke-dasharray для прогноза. Промежуточные фазы (фолликулярная, лютеиновая) - точка 8x8 под числом, opacity 0.65 для прогноза. Сверху календаря - легенда фаз. Решение опирается на feedback Ирены: сохранить 4 цвета фаз (не только менструация и овуляция как у FLO).
...
**Дополнительно выполнено вечером 9 мая:**

- **Карточка "Цикл" добавлена в hub-аппу studio** (commit af1fbbe в репозитории studio). Иконка 🌸, цвет розовый из палитры cycle (`#e8a5a0`), URL `t.me/relax2000_bot/cycle` через функцию `openApp`. Реальная точка входа для пользовательниц Ирены.

- **История получила цветные ленты фаз под каждой записью.** Точки по дням цикла, цвет = фаза. Овуляция - контурные сердечки SVG. Текущий цикл показывает прошедшие дни в фазах + серые точки до avgLength. Завершённые циклы используют свою фактическую длительность для расчёта фаз. Согласовано с календарём.

**Post-MVP по приоритету:**

a) **`getCycleContext(userId)` в `biohack/src/App.js`.** Системный промпт ИИ-коуча получает текущую фазу пользовательницы для персонализации советов (например при овуляции советует одно, при лютеиновой - другое). Файл: `../biohack/src/App.js` (read-only зона).

b) **Описания под медитациями в studio.** Ждёт тексты от Vladlen.

c) **Карточки анализов порции 3-5.** Knowledge base ИИ-подружки.

d) **Большой анонс-пост в платный канал @relax2000.** Про обновления экосистемы (новый Цикл, напоминание про studio). Стиль постов Ирены: эмоциональный, тёплый, с эмодзи (девочки, наконец-то, готово). Использовать `tg-sender.html` с inline кнопками.

e) **Календарь до конца года.** Вместо "бесконечного скролла" рассматриваем фиксированно 12 месяцев вперёд от текущего, или до декабря текущего года. Даёт 95% пользы при значительно меньшей сложности.

f) **Tap-info на день в календаре.** Плашка при тапе с инфой ("9 мая, 1-й день цикла, менструация"). Low priority, отложили после UX-апгрейда календаря.

g) **Studio pending UX-правки** (другой репозиторий). Убрать или минимизировать "IRENA.BIO" заголовок (мало места под контент), плеер медитаций не помещается на экран целиком в Telegram WebView, медитации не помещаются на экран без избыточного скролла. Все правки в `../studio/index.html` - открывать Claude Code в самом studio проекте.

h) **Чистка БД от тестового user_id** если решит использовать один аккаунт. На стадии разработки могло накопиться несколько тестовых записей с разных аккаунтов.
...
>
> Сейчас работаем над [пункт из секции 10, например "(d) большой анонс-пост в платный канал"].
